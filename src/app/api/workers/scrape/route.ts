...
export async function POST(request: Request) {
  ...
  try {
    console.log(`[Worker] Starting scrape for ${domainUrl}`);
    ...
    console.log(`[Worker] Successfully completed scrape for ${domainUrl}`);

    return NextResponse.json({ success: true });

  } catch (error: unknown) {
    console.error(`[Worker] Scrape failed for ${domainUrl}:`, error);
